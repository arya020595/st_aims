...
const resolvers = {
  Query: {
    allLivestockCategories: async (self, params, context) => {
      ...
      if (!role || !role.privileges.includes("Category Livestock:Read")) {
        return [];
      }

      let queryResult = await context.prisma.livestockCategory.findMany({
        where: {
    ...
    allLivestockCategoriesEncrypted: async (self, params, context) => {
      assertValidSession(context.activeSession);
  ...
  Mutation: {
    createLivestockCategory: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.livestockCategory.create({
        data: {
          ...createPayload,
        },
      });
    ...
    updateLivestockCategory: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.livestockCategory.update({
        where: {
          uuid: payload.uuid,
        },
        data: {
    ...
    deleteLivestockCategory: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      let getDeletedData = await context.prisma.livestockCategory.findUnique({
        where: {
          uuid: payload.uuid,
        },
      });
    ...
    exportLivestockCategory: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      let categories = await context.prisma.livestockCategory.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
        orderBy: {
      ...
      let productionSheet = workbook.addWorksheet("LiveStock Category");

      let headerRow = ["CODE", "CATEGORY NAME"];
      ...
      const filename = `livestock_category.xlsx`;
      const fileUrl =
