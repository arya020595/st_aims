...
const resolvers = {
  Query: {
    allMilledRiceLocations: async (self, params, context) => {
      ...
      if (
        ...
        !role.privileges.includes(
          "Milled Rice Location Paddy Master Data Crops:Read"
        )
      ) {
        return [];
      }

      let queryResult = await context.prisma.milledRiceLocation.findMany({
        where: {
    ...
    tokenizedAllMilledRiceLocations: async (self, params, context) => {
      assertValidSession(context.activeSession);
  ...
  Mutation: {
    createMilledRiceLocation: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.milledRiceLocation.create({
        data: {
          ...createPayload,
        },
      });
    ...
    updateMilledRiceLocation: async (self, params, context) => {
      assertValidSession(context.activeSession);

      await context.prisma.milledRiceLocation.update({
        where: {
          uuid: params.uuid,
        },
        data: {
    ...
    deleteMilledRiceLocation: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData = await context.prisma.milledRiceLocation.findUnique({
        where: {
          uuid: params.uuid,
        },
      });
    ...
    tokenizedCreateMilledRiceLocation: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateMilledRiceLocation: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteMilledRiceLocation: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    exportMilledRiceLocation: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      milledRiceLocations = milledRiceLocations.map((q) => {
        let id = BigInt(q.id);
        return {
          ...q,
          id: id.toString(),
      ...
      let productionSheet = workbook.addWorksheet("Milled Rice Location");

      let headerRow = ["LOCATION", "DESCRIPTION"];
      ...
      for (const milledRiceLocation of milledRiceLocations) {
        excelHelper.addText({
          sheet: productionSheet,
          row: rowCounter,
          col: ++colCounter,
          value: milledRiceLocation?.location || "",
          alignment: {
            vertical: "middle",
            horizontal: "left",
          },
        ...
        excelHelper.addText({
          ...
          row: rowCounter,
          col: ++colCounter,
          value: milledRiceLocation?.description || "",
          alignment: {
            vertical: "middle",
            horizontal: "left",
          },
      ...
      const filename = `milled_rice_location.xlsx`;
      const fileUrl =
