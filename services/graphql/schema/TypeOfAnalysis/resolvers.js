...
const resolvers = {
  Query: {
    allTypeOfAnalysises: async (self, params, context) => {
      ...
      if (!role || !role.privileges.includes("Type Of Analysis:Read")) {
        return [];
      }

      let queryResult = await context.prisma.typeOfAnalysis.findMany({
        where: {
    ...
    countTypeOfAnalysis: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedAllTypeOfAnalysises: async (self, params, context) => {
      assertValidSession(context.activeSession);
  ...
  Mutation: {
    createTypeOfAnalysis: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.typeOfAnalysis.create({
        data: {
          ...createPayload,
        },
      });
    ...
    updateTypeOfAnalysis: async (self, params, context) => {
      assertValidSession(context.activeSession);

      await context.prisma.typeOfAnalysis.update({
        where: {
          uuid: params.uuid,
        },
        data: {
    ...
    deleteTypeOfAnalysis: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData = await context.prisma.typeOfAnalysis.findUnique({
        where: {
          uuid: params.uuid,
        },
      });
    ...
    tokenizedCreateTypeOfAnalysis: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateTypeOfAnalysis: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteTypeOfAnalysis: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    exportTypeOfAnalysis: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      let allTypeOfAnalysis = await context.prisma.typeOfAnalysis.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
      });
      ...
      let productionSheet = workbook.addWorksheet("Type Of Analysis");

      let headerRow = ["TYPE OF ANALYSIS NAME"];
      ...
      for (const typeOfAnalysis of allTypeOfAnalysis) {
        excelHelper.addText({
          sheet: productionSheet,
          row: rowCounter,
          col: ++colCounter,
          value: typeOfAnalysis?.name || "",
          alignment: {
            vertical: "middle",
            horizontal: "left",
          },
      ...
      const filename = `type_of_analysis.xlsx`;
      const fileUrl =
