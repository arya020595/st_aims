...
const resolvers = {
  Query: {
    allFruitProductionEstimateds: async (self, params, context) => {
      ...
      if (
        !role ||
        !role.privileges.includes("Estimate Production Fruit:Read")
      ) {
        return [];
      }
      ...
      if (profileQuery.rocbnRegNo) {
        ...
        const found = await context.prisma.fruitProductionEstimated.findMany({
          where: {
            farmerUUID: {
        ...
        let queryResult =
          await context.prisma.fruitProductionEstimated.findMany({
            where: {
              ...filterQuery,
              ...NOT_DELETED_DOCUMENT_QUERY,
            },
    ...
    tokenizedAllFruitProductionEstimateds: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      if (profileQuery.rocbnRegNo) {
        ...
        // let countFruitEstimateProduction = await context.prisma.fruitProductionEstimated.findMany({
        //   where: {
        //     farmerUUID: {
        //       in: getCompany.map((c) => c.uuid),
        //     },
        ...
        // countFruitEstimateProduction = countFruitEstimateProduction.length

        // const pages = Math.ceil(countFruitEstimateProduction / params.pageSize)

        let found = await context.prisma.fruitProductionEstimated.findMany({
          where: {
            farmerUUID: {
              in: getCompany.map((c) => c.uuid),
            },
    ...
    countAllFruitProductionEstimateds: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      if (profileQuery.rocbnRegNo) {
        ...
        let found = await context.prisma.fruitProductionEstimated.count({
          where: {
            farmerUUID: {
              in: getCompany.map((c) => c.uuid),
            },
        ...
        let queryResult = await context.prisma.fruitProductionEstimated.count({
          where: {
            ...filterQuery,
            ...NOT_DELETED_DOCUMENT_QUERY,
          },
  ...
  Mutation: {
    createFruitProductionEstimated: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.fruitProductionEstimated.create({
        data: {
          ...createPayload,
        },
      });
      ...
      await context.prisma.activityLogs.create({
        data: {
          uuid: uuidv4(),
          type: "CREATE",
          tableName: "fruitProductionEstimated",
          log: {
            ...createPayload,
          },
        },
    ...
    updateFruitProductionEstimated: async (self, params, context) => {
      assertValidSession(context.activeSession);

      await context.prisma.fruitProductionEstimated.update({
        where: {
          uuid: params.uuid,
        },
        data: {
    ...
    deleteFruitProductionEstimated: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData =
        await context.prisma.fruitProductionEstimated.findUnique({
          where: {
            uuid: params.uuid,
          },
        });
    ...
    exportFruitProductionEstimated: async (sel, params, context) => {
      assertValidSession(context.activeSession);
      ...
      if (
        ...
        productions = await context.prisma.fruitProductionEstimated.findMany({
          where: {
            ...filterQuery,
            ...NOT_DELETED_DOCUMENT_QUERY,
            farmerUUID: {
      ...
      const indexedFruit = fruit.reduce((all, fru) => {
        if (!all[fru.uuid]) {
          all[fru.uuid] = {};
        }
        all[fru.uuid] = fru;
      ...
      let productionSheet = workbook.addWorksheet("Fruit Production Estimated");
      ...
      for (const prod of productions) {
        const fruit = indexedFruit[prod.fruitUUID];
      ...
      // }
      // const filename = `fruit_production_estimate.xlsx`;
      // const fileUrl =
    ...
    tokenizedCreateFruitProductionEstimated: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateFruitProductionEstimated: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteFruitProductionEstimated: async (self, params, context) => {
      assertValidSession(context.activeSession);
