...
const resolvers = {
  Query: {
    allTests: async (self, params, context) => {
      ...
      if (!role || !role.privileges.includes("Test Agrifood:Read")) {
        return [];
      }

      let queryResult = await context.prisma.test.findMany({
        where: {
    ...
    countTest: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedAllTests: async (self, params, context) => {
      assertValidSession(context.activeSession);
  ...
  Mutation: {
    createTest: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.test.create({
        data: {
          uuid: uuidv4(),
          ...params,
          createdAt: new Date().toISOString(),
    ...
    updateTest: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    deleteTest: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData = await context.prisma.test.findUnique({
        where: {
          uuid: params.uuid,
        },
      });
    ...
    tokenizedCreateTest: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateTest: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteTest: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    exportsTest: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      let tests = await context.prisma.test.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
      });
      ...
      let productionSheet = workbook.addWorksheet("Test");

      let headerRow = ["TEST ID", "TEST NAME"];
