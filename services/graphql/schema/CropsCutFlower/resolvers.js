...
const resolvers = {
  Query: {
    allCropsCutFlowers: async (self, params, context) => {
      ...
      if (!role || !role.privileges.includes("Cut Flower Floriculture:Read")) {
        return [];
      }

      let queryResult = await context.prisma.cutFlower.findMany({
        where: {
    ...
    tokenizedAllCropsCutFlowers: async (self, params, context) => {
      assertValidSession(context.activeSession);
  ...
  Mutation: {
    createCropsCutFlower: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let countData = await context.prisma.cutFlower.count();
      ...
      await context.prisma.cutFlower.create({
        data: {
          ...createPayload,
        },
      });
    ...
    updateCropsCutFlower: async (self, params, context) => {
      assertValidSession(context.activeSession);

      await context.prisma.cutFlower.update({
        where: {
          uuid: params.uuid,
        },
        data: {
    ...
    deleteCropsCutFlower: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData = await context.prisma.cutFlower.findUnique({
        where: {
          uuid: params.uuid,
        },
      });
    ...
    tokenizedCreateCropsCutFlower: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateCropsCutFlower: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteCropsCutFlower: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    exportsCropsCutFlower: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      let productionSheet = workbook.addWorksheet("Cut Flower");
      ...
      for (const cutFlower of cutFlowers) {
        excelHelper.addText({
          sheet: productionSheet,
          row: rowCounter,
          col: ++colCounter,
      ...
      const filename = `cut_flower.xlsx`;
      const fileUrl =
