...
const resolvers = {
  Query: {
    allUnits: async (self, params, context) => {
      ...
      if (
        !role ||
        (!role.privileges.includes("Livestock Unit:Read") &&
          !role.privileges.includes("Unit:Read"))
      ) {
        return [];
      }

      let queryResult = await context.prisma.unit.findMany({
        where: {
    ...
    countUnit: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedAllUnits: async (self, params, context) => {
      assertValidSession(context.activeSession);
      // let user = await context.prisma.user.findUnique({
      //   where: {
      //     uuid: context.activeSession.User.userId,
  ...
  Mutation: {
    createUnit: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.unit.create({
        data: {
          ...createPayload,
        },
      });
    ...
    updateUnit: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.unit.update({
        where: {
          uuid: payload.uuid,
        },
        data: {
    ...
    deleteUnit: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      let getDeletedData = await context.prisma.unit.findUnique({
        where: {
          uuid: payload.uuid,
        },
      });
    ...
    exportUnit: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      let units = await context.prisma.unit.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
      });
      ...
      let productionSheet = workbook.addWorksheet("Unit");

      let headerRow = ["UNIT NAME", "UNIT DESCRIPTION"];
