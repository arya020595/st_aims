...
const resolvers = {
  Query: {
    allBioSecurityTakenActions: async (self, params, context) => {
      ...
      if (!role || !role.privileges.includes("Taken Action:Read")) {
        return [];
      }

      let queryResult = await context.prisma.bioSecurityTakenAction.findMany({
        where: {
    ...
    tokenizedAllBioSecurityTakenActions: async (self, params, context) => {
      assertValidSession(context.activeSession);
  ...
  Mutation: {
    createBioSecurityTakenAction: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.bioSecurityTakenAction.create({
        data: {
          ...createPayload,
        },
      });
      ...
      await context.prisma.activityLogs.create({
        data: {
          uuid: uuidv4(),
          type: "CREATE",
          tableName: "bioSecurityTakenAction",
          log: {
            ...createPayload,
          },
        },
    ...
    updateBioSecurityTakenAction: async (self, params, context) => {
      assertValidSession(context.activeSession);

      await context.prisma.bioSecurityTakenAction.update({
        where: {
          uuid: params.uuid,
        },
        data: {
    ...
    deleteBioSecurityTakenAction: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData =
        await context.prisma.bioSecurityTakenAction.findUnique({
          where: {
            uuid: params.uuid,
          },
        });
    ...
    tokenizedCreateBioSecurityTakenAction: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateBioSecurityTakenAction: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteBioSecurityTakenAction: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    exportBioSecurityTakenAction: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      let takenActions = await context.prisma.bioSecurityTakenAction.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
      });
      ...
      let productionSheet = workbook.addWorksheet("Taken Action");
      ...
      for (const takenAction of takenActions) {
        excelHelper.addText({
          sheet: productionSheet,
          row: rowCounter,
          col: ++colCounter,
          value: takenAction?.name || "",
          alignment: {
            vertical: "middle",
            horizontal: "left",
          },
      ...
      const filename = `taken_action.xlsx`;
      const fileUrl =
