...
const resolvers = {
  Query: {
    allCropsPaddyByProducts: async (self, params, context) => {
      ...
      if (
        !role ||
        !role.privileges.includes("By Product Paddy Master Data Crops:Read")
      ) {
        return [];
      }

      let queryResult = await context.prisma.cropsPaddyByProduct.findMany({
        where: {
    ...
    tokenizedAllCropsPaddyByProducts: async (self, params, context) => {
      assertValidSession(context.activeSession);
  ...
  Mutation: {
    createCropsPaddyByProduct: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let countData = await context.prisma.cropsPaddyByProduct.count();
      ...
      const cropsPaddyByProductId = startCode;

      const createPayload = {
        uuid: uuidv4(),
        ...params,
        typeOfByProductId: cropsPaddyByProductId,
      ...
      await context.prisma.cropsPaddyByProduct.create({
        data: {
          ...createPayload,
        },
      });
    ...
    updateCropsPaddyByProduct: async (self, params, context) => {
      assertValidSession(context.activeSession);

      await context.prisma.cropsPaddyByProduct.update({
        where: {
          uuid: params.uuid,
        },
        data: {
    ...
    deleteCropsPaddyByProduct: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData = await context.prisma.cropsPaddyByProduct.findUnique({
        where: {
          uuid: params.uuid,
        },
      });
    ...
    tokenizedCreateCropsPaddyByProduct: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateCropsPaddyByProduct: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteCropsPaddyByProduct: async (self, params, context) => {
      let userId = "";
    ...
    exportsCropsPaddyByProduct: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      let byProducts = await context.prisma.cropsPaddyByProduct.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
      });

      byProducts = byProducts.map((q) => {
        let id = BigInt(q.id);
        return {
          ...q,
          id: id.toString(),
      ...
      let productionSheet = workbook.addWorksheet("By Product");

      let headerRow = ["TYPE OF BY-PRODUCT", "PRICE/KG"];
      ...
      for (const byProduct of byProducts) {
        excelHelper.addText({
          ...
          row: rowCounter,
          col: ++colCounter,
          value: byProduct?.typeOfByProduct || "",
          alignment: {
            vertical: "middle",
            horizontal: "left",
          },
      ...
      const filename = `by_product.xlsx`;
      const fileUrl =
