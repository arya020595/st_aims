...
const resolvers = {
  Query: {
    allBioSecurityUnits: async (self, params, context) => {
      ...
      if (!role || !role.privileges.includes("Unit Bio Security:Read")) {
        return [];
      }

      let queryResult = await context.prisma.bioSecurityUnit.findMany({
        where: {
    ...
    tokenizedAllBioSecurityUnits: async (self, params, context) => {
      assertValidSession(context.activeSession);
  ...
  Mutation: {
    createBioSecurityUnit: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.bioSecurityUnit.create({
        data: {
          ...createPayload,
        },
      });
    ...
    updateBioSecurityUnit: async (self, params, context) => {
      assertValidSession(context.activeSession);

      await context.prisma.bioSecurityUnit.update({
        where: {
          uuid: params.uuid,
        },
        data: {
    ...
    deleteBioSecurityUnit: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData = await context.prisma.bioSecurityUnit.findUnique({
        where: {
          uuid: params.uuid,
        },
      });
    ...
    tokenizedCreateBioSecurityUnit: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateBioSecurityUnit: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteBioSecurityUnit: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    exportBioSecurityUnit: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      let units = await context.prisma.bioSecurityUnit.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
      });
      ...
      let productionSheet = workbook.addWorksheet("BioSecurity Unit");
      ...
      const filename = `biosecurity_unit.xlsx`;
      const fileUrl =
