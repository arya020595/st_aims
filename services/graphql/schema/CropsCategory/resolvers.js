...
const resolvers = {
  Query: {
    allCropsCategories: async (self, params, context) => {
      ...
      if (!role || !role.privileges.includes("Category Master Data:Read")) {
        return [];
      }

      let queryResult = await context.prisma.cropsCategory.findMany({
        where: {
    ...
    tokenizedAllCropsCategories: async (self, params, context) => {
      assertValidSession(context.activeSession);
  ...
  Mutation: {
    createCropsCategory: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.cropsCategory.create({
        data: {
          ...createPayload,
        },
      });
    ...
    updateCropsCategory: async (self, params, context) => {
      assertValidSession(context.activeSession);

      await context.prisma.cropsCategory.update({
        where: {
          uuid: params.uuid,
        },
        data: {
    ...
    deleteCropsCategory: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData = await context.prisma.cropsCategory.findUnique({
        where: {
          uuid: params.uuid,
        },
      });
    ...
    tokenizedCreateCropsCategory: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateCropsCategory: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteCropsCategory: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    exportCropsCategory: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      let categorys = await context.prisma.cropsCategory.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
        // orderBy: {
      ...
      let productionSheet = workbook.addWorksheet("Category");

      let headerRow = ["ID", "NAME", "DESCRIPTION", "PREFIX CODE"];
