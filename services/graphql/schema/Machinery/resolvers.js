...
const resolvers = {
  Query: {
    allMachineries: async (self, params, context) => {
      ...
      if (!role || !role.privileges.includes("Machinery Agrifood:Read")) {
        return [];
      }
    ...
    countMachineries: async (self, params, context) => {
      const queryResult = await context.prisma.machinery.findMany({
        where: {},
        orderBy: {
          id: "desc",
    ...
    tokenizedAllMachineries: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      let queryResult = await context.prisma.machinery.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
        orderBy: {
  ...
  Mutation: {
    createMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.machinery.create({
        data: {
          ...createPayload,
        },
      });
    ...
    updateMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);

      await context.prisma.machinery.update({
        where: {
          uuid: params.uuid,
        },
        data: {
    ...
    deleteMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData = await context.prisma.machinery.findUnique({
        where: {
          uuid: params.uuid,
        },
      });
    ...
    tokenizedCreateMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    exportMachinery: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      let machineries = await context.prisma.machinery.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
      });

      machineries = machineries.map((q) => {
        let id = BigInt(q.id);
        return {
          ...q,
          id: id.toString(),
      ...
      let productionSheet = workbook.addWorksheet("Machinery");
      ...
      for (const machinery of machineries) {
        ...
        excelHelper.addText({
          ...
          row: rowCounter,
          col: ++colCounter,
          value: machinery?.machineCapacity || "",
          alignment: {
            vertical: "middle",
            horizontal: "left",
          },
