...
const resolvers = {
  Query: {
    allBioSecurityCompliances: async (self, params, context) => {
      ...
      if (!role || !role.privileges.includes("Compliance:Read")) {
        return [];
      }

      let queryResult = await context.prisma.bioSecurityCompliance.findMany({
        where: {
    ...
    tokenizedAllBioSecurityCompliances: async (self, params, context) => {
      assertValidSession(context.activeSession);
  ...
  Mutation: {
    createBioSecurityCompliance: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.bioSecurityCompliance.create({
        data: {
          ...createPayload,
        },
      });
      ...
      await context.prisma.activityLogs.create({
        data: {
          uuid: uuidv4(),
          type: "CREATE",
          tableName: "bioSecurityCompliance",
          log: {
            ...createPayload,
          },
        },
    ...
    updateBioSecurityCompliance: async (self, params, context) => {
      assertValidSession(context.activeSession);

      await context.prisma.bioSecurityCompliance.update({
        where: {
          uuid: params.uuid,
        },
        data: {
    ...
    deleteBioSecurityCompliance: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData =
        await context.prisma.bioSecurityCompliance.findUnique({
          where: {
            uuid: params.uuid,
          },
        });
    ...
    tokenizedCreateBioSecurityCompliance: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateBioSecurityCompliance: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteBioSecurityCompliance: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    exportBioSecurityCompliance: async (self, params, context) => {
      const foundUser = await context.prisma.user.findUnique({
        where: {
          uuid: context.activeSession.User.userId,
        },
      ...
      let compliances = await context.prisma.bioSecurityCompliance.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
      });
      ...
      let productionSheet = workbook.addWorksheet("Compliance");
