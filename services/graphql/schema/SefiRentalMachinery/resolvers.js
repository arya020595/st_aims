...
const resolvers = {
  Query: {
    allSefiRentalMachineries: async (self, params, context) => {
      ...
      if (!role || !role.privileges.includes("SEFI Rental Machinaries:Read")) {
        return [];
      }

      let queryResult = await context.prisma.sefiRentalMachinery.findMany({
        where: {
    ...
    countSefiRentalMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedAllSefiRentalMachineries: async (self, params, context) => {
      assertValidSession(context.activeSession);
  ...
  Mutation: {
    createSefiRentalMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);
      ...
      await context.prisma.sefiRentalMachinery.create({
        data: {
          ...createPayload,
        },
      });
    ...
    updateSefiRentalMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);

      await context.prisma.sefiRentalMachinery.update({
        where: {
          uuid: params.uuid,
        },
        data: {
    ...
    deleteSefiRentalMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);

      let getDeletedData = await context.prisma.sefiRentalMachinery.findUnique({
        where: {
          uuid: params.uuid,
        },
      });
    ...
    tokenizedCreateSefiRentalMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedUpdateSefiRentalMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    tokenizedDeleteSefiRentalMachinery: async (self, params, context) => {
      assertValidSession(context.activeSession);
    ...
    exportSefiRental: async (self, params, context) => {
      ...
      let sefiRentals = await context.prisma.sefiRentalMachinery.findMany({
        where: {
          ...NOT_DELETED_DOCUMENT_QUERY,
        },
        orderBy: {
      ...
      let productionSheet = workbook.addWorksheet("Sefi Rental Machineries");

      let headerRow = ["MACHINE ID", "MACHINE NAME"];
      ...
      const filename = `sefi_rental_machineries.xlsx`;
      const fileUrl =
