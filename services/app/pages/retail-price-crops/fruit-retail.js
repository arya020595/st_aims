...
import Fruit from "../../components/RetailPriceCropsImport/Fruit";
...
const QUERY = gql`
  query allFruitRetailPrices($monthYear: String!) {
    countAllFruitRetailPrices(monthYear: $monthYear)
...
const CREATE_RETAILPRICE = gql`
  mutation tokenizedCreateManyFruitRetailPrice($tokenized: String!) {
    tokenizedCreateManyFruitRetailPrice(tokenized: $tokenized)
  }
`;

const UPDATE_RETAILPRICE = gql`
  mutation tokenizedUpdateFruitRetailPrice($tokenized: String!) {
    tokenizedUpdateFruitRetailPrice(tokenized: $tokenized)
  }
`;

const DELETE_RETAILPRICE = gql`
  mutation tokenizedDeleteFruitRetailPrice($tokenized: String!) {
    tokenizedDeleteFruitRetailPrice(tokenized: $tokenized)
  }
`;

const EXPORT_FRUIT_RETAILRPICE = gql`
  mutation exportFruitRetailPrice($monthYear: String!) {
    exportFruitRetailPrice(monthYear: $monthYear)
  }
`;
...
const page = () => {
  ...
  const [createFruitRetailPrice] = useMutation(CREATE_RETAILPRICE);
  const [updateFruitRetailPrice] = useMutation(UPDATE_RETAILPRICE);
  const [deleteFruitRetailPrice] = useMutation(DELETE_RETAILPRICE);
  const [exportFruitRetailPrice] = useMutation(EXPORT_FRUIT_RETAILRPICE);
  ...
  let [countFruitRetailPrice, setCountFruitRetailPrice] = useState(0);
  ...
  let pageCount = useMemo(() => {
    if (!countFruitRetailPrice) return 1;
    return Math.ceil(countFruitRetailPrice / pageSize);
  }, [countFruitRetailPrice, pageSize]);
  ...
  const [allFruitRetailPrices, setAllFruitRetailPrices] = useState([]);
  ...
  useEffect(() => {
    if (!loading && !error) {
      // const encryptedFruitRetailPrices = data?.tokenizedAllFruitRetailPrices || "";
      // let allFruitRetailPrices = [];
      // if (encryptedFruitRetailPrices) {
      //   const decrypted = jwt.verify(encryptedFruitRetailPrices, TOKENIZE);
      //   allFruitRetailPrices = decrypted.queryResult;
      //   setAllFruitRetailPrices(allFruitRetailPrices);
      // }
      ...
      // }
      const countData = data?.countAllFruitRetailPrices || 0;
      setCountFruitRetailPrice(countData);
    }
  }, [data, loading, error, savedCount]);

  const [selectedMultipleFruit, setSelectedMultipleFruit] = useState([]);
  // const availableOptions = allCropsFruits.filter(
  //   (comm) => !selectedMultipleFruit.includes(comm.uuid)
  // );

  useEffect(() => {
    let selectedOption = formData.records
      .filter((Fruit) => Fruit.fruitUUID)
      .map((veg) => veg.fruitUUID);
    setSelectedMultipleFruit(selectedOption);
  }, [formData.records]);
  ...
  const fetchingVegetable = async (input, callback) => {
    ...
    let allCropsFruitsQuery = [];
    const encryptedFruit = result.data?.searchAllCropsFruits;
    if (encryptedFruit) {
      const decrypted = jwt.verify(encryptedFruit, TOKENIZE);
      allCropsFruitsQuery = decrypted.queryResult;
    }
    const filterFruit = allCropsFruitsQuery.filter(
      (fru) => !selectedMultipleFruit.includes(fru.uuid)
    );
    setAllCropsFruits(allCropsFruitsQuery);
    callback(filterFruit);
  };
  ...
  useEffect(async () => {
    ...
    const encryptedFruitRetailPrices =
      result.data?.tokenizedAllFruitRetailPrices || "";
    let allFruitRetailPrices = [];
    if (encryptedFruitRetailPrices) {
      const decrypted = jwt.verify(encryptedFruitRetailPrices, TOKENIZE);
      allFruitRetailPrices = decrypted.queryResult;
  ...
  // const encryptedFarmLocations = data?.tokenizedAllFarmLocations || "";
      ...
      <FormModal
        ...
        onSubmit={async (e) => {
          ...
          try {
            ...
            delete payload.Fruit;
            const tokenized = jwt.sign(payload, TOKENIZE);
            await updateFruitRetailPrice({
              variables: {
                // ...updateFormData,
                tokenized,
              },
          ...
          <AsyncSelect
            ...
            onChange={(selectedValues) => {
              setUpdateFormData({
                ...
                fruitUUID: selectedValues.uuid,
                fruitId: selectedValues?.fruitId || "",
                Fruit: selectedValues,
              });
            }}
          />
          {/* <Select
      ...
      <FormModal
        ...
        onSubmit={async (e) => {
          ...
          try {
            ...
            //   if (!tokenizedPayload.fruitUUID) {
            //     throw {
            //       message: "Invalid Fruit Name",
            //     };
            //   }
            //   if (!tokenizedPayload.fruitId) {
            //     throw {
            //       message: "Invalid Fruit ID",
            //     };
            //   }
            ...
            <div>
              <AsyncSelect
                ...
                onChange={(selectedValues) => {
                  ...
                  setSelectedFruit(selectedValues);
                }}
              />
              {/* <Select
                value={{
      ...
      <div className="mt-26 pr-0 md:pr-10 py-4 h-full">
        ...
        {importForm ? (
          ...
          <TableAsync
                ...
                <div className="flex mx-2 items-end">
                  <button
                    ...
                    onClick={async (e) => {
                      ...
                      showLoadingSpinner();
                      try {
                        const response = await exportFruitRetailPrice({
                          variables: {
                            monthYear: yearMonth,
                          },
                        });
                        ...
                        const base64Response =
                          response.data.exportFruitRetailPrice;
                        ...
                        link.href = url;
                        link.download = "fruit_retail_price.xlsx";
                        link.click();
            ...
            withoutHeader={true}
            customUtilities={
              !currentUserDontHavePrivilege(["Fruit Retail Price:Update"])
                ? customUtilities
                : null
            }
            customUtilitiesPosition="left"
            onAdd={
              !currentUserDontHavePrivilege(["Fruit Retail Price:Create"])
                ? () => {
                    setFormData({
                      records: [],
                    });
            ...
            onRemove={
              !currentUserDontHavePrivilege(["Fruit Retail Price:Delete"])
                ? async ({ rows }) => {
                    showLoadingSpinner();
                    try {
                      let yes = confirm(
                        `Are you sure to delete ${rows.length} Fruit Retail Price?`
                      );
                      if (yes) {
                        for (const row of rows) {
                          const tokenized = jwt.sign(row, TOKENIZE);
                          deleteFruitRetailPrice({
                            variables: {
                              // uuid: row.uuid,
                              tokenized,
                            },
                        ...
                        notification.addNotification({
                          title: "Success!",
                          message: `${rows.length} Fruit Retail Price deleted`,
                          level: "success",
                        });
                        await refetch();
                        setSavedCount((savedCount += 1));
