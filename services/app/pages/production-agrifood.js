...
const PRODUCT_CATALOGUE_DETAILS_QUERY = gql`
...
const CREATE_PRODUCTION = gql`
  mutation tokenizedCreateManyAgrifoodProduction($tokenized: String!) {
    tokenizedCreateManyAgrifoodProduction(tokenized: $tokenized)
  }
`;
...
const page = () => {
  ...
  const [tableProductVisible, setTableProductVisible] = useState(false);
  const [tableProductFormData, setTableProductFormData] = useState({
  ...
  const columnsProduct = useMemo(() => [
    ...
    },
    {
      ...
      accessor: "uuid",
      Cell: (props) => {
        const selected = formData.records.find(
          (r) => r.productCatalogueUUID === props.row?.original?.uuid
        );
        return (
          <div>
            <button
              className={`rounded-md px-4 py-2 text-white ${
                selected
                  ...
                  : "bg-mantis-500 hover:bg-mantis-700"
              } `}
              onClick={(e) => {
                if (e) e.preventDefault();
                if (!formData.uuid) {
                  if (selected) {
                    setFormData({
                      ...
                      records: formData.records.filter(
                        (r) =>
                          r.productCatalogueUUID === props.row?.original?.uuid
                      ),
                    });
                  } else {
                    setFormData({
                      ...
                      records: [
                        ...formData.records,
                        {
                          uuid: uuidv4(),
                          productCategoryUUID:
                            props.row?.original?.productCategoryUUID,
                          productCategory: props.row?.original?.productCategory,
                          productSubCategoryUUID:
                            props.row?.original?.productSubCategoryUUID,
                          productSubCategory:
                            props.row?.original?.productSubCategory,
                          productName: props.row?.original?.name,
                          productCatalogueUUID: props.row?.original?.uuid,
                          priceLists: props.row?.original?.priceLists || [],
                          netWeight: props.row?.original?.weight || 0,
                          actualUnit: 0,
                          pricePerUnit: 0,
                          quantity: 0,
                  ...
                  setFormData({
                    ...
                    productSubCategoryUUID:
                      props.row?.original?.productSubCategoryUUID,
                    productSubCategory: props.row?.original?.productSubCategory,
                    productName: props.row?.original?.name,
                    productCatalogueUUID: props.row?.original?.uuid,
                    priceLists: props.row?.original?.priceLists || [],
                    pricePerUnit: 0,
    ...
    <AdminArea header={{ title: "Production" }} urlQuery={router.query}>
      ...
      >
        <div className="flex mb-2">
          ...
          <p className="text-2xl font-bold mx-2 text-mantis-600">
            {tableProductFormData.CompanyProfile?.companyName || ""}
          </p>
        </div>
        <div className="flex mb-2">
          <p className="text-2xl">Company Reg Number : </p>
          <p className="text-2xl font-bold mx-2 text-mantis-600">
            {tableProductFormData.CompanyProfile?.companyRegNo || ""}
          </p>
        </div>
        <Table
          columns={columnsProduct}
          data={tableProductFormData.productCatalogueDetails}
          withoutHeader={true}
        />
      </FormModal>
      <FormModal
        ...
        onSubmit={async (e) => {
          ...
          try {
            ...
            const base64Response = response.data.exportAgrifoodProduction;
            ...
            link.href = url;
            link.download = "agrifood_production.xlsx";
            link.click();
      ...
      >
        <div className="form-group">
          ...
          <input
            required
            type="month"
            className={`form-control uppercae${!formData.uuid ? "w-1/5" : ""}`}
        ...
        {!formData.uuid ? (
          <div>
            ...
            <div className="flex justify-start mb-4">
              ...
              >
                <i className="fa fa-plus" /> Search
              </button> */}
              <button
                className="bg-purple-500 col-span-3 rounded-md text-white py-2 px-3 shadow-md"
                onClick={async (e) => {
                  if (e) e.preventDefault();
                  ...
                  try {
                    ...
                    if (!formData.premiseUUID) {
                      throw {
                        message: "Please Select Premise Address",
                      };
                    }

                    const payloadTokenized = {
                      companyUUID: formData.companyUUID,
                      // premiseUUID: formData.premiseUUID,
                    };
                    ...
                    const result = await client.query({
                      query: PRODUCT_CATALOGUE_DETAILS_QUERY,
                      variables: { tokenizedParams },
                      fetchPolicy: "no-cache",
                    });

                    let productCatalogueDetailsByCompany = [];
                    ...
                    if (encryptedProductCatalogueDetails) {
                      ...
                      productCatalogueDetailsByCompany = decrypted.queryResult;
                    }
                    ...
                    const encryptedAgrifoodCompanyProfiles =
                      query.data?.searchAllAgrifoodCompanyProfiles || "";
                    let companyQuery = [];
                    if (encryptedAgrifoodCompanyProfiles) {
                      const decrypted = jwt.verify(
                        encryptedAgrifoodCompanyProfiles,
                      ...
                      companyQuery = decrypted.queryResult;
                    }
                    setTableProductFormData({
                      ...tableProductFormData,
                      CompanyProfile: companyQuery[0],
                      premiseAddress: formData.premiseAddress,
                      productCatalogueDetails: productCatalogueDetailsByCompany,
                    });

                    setModalVisible(false);
                    setTableProductVisible(true);
                  } catch (error) {
                    notification.handleError(error);
                  }
                }}
              >
                <i className="fa fa-search" /> Select and Search Product
              </button>
            </div>
          ...
          <div>
            ...
            <div>
              <button
                className="bg-purple-500 w-full rounded-sm text-white py-1 mt-1 shadow-md"
                onClick={async (e) => {
                  if (e) e.preventDefault();
