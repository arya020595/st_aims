...
const CREATE_UNIT = gql`
  mutation createUnit($tokenized: String!) {
    createUnit(tokenized: $tokenized)
  }
`;

const UPDATE_UNIT = gql`
  mutation updateUnit($tokenized: String!) {
    updateUnit(tokenized: $tokenized)
  }
`;

const DELETE_UNIT = gql`
  mutation deleteUnit($tokenized: String!) {
    deleteUnit(tokenized: $tokenized)
  }
`;
    ...
    <AdminArea header={{ title: "Unit" }} urlQuery={router.query}>
      ...
      <div className="mt-26 pr-0 md:pr-10 py-4 h-full">
        <Table
          ...
          customUtilities={
            !currentUserDontHavePrivilege(["Unit:Update"])
              ? customUtilities
              : null
          }
          onAdd={
            !currentUserDontHavePrivilege(["Unit:Create"])
              ? () => {
                  setFormData({});
                  setModalVisible(true);
                }
          ...
          onRemove={
            !currentUserDontHavePrivilege(["Unit:Delete"])
              ? async ({ rows }) => {
                  showLoadingSpinner();
                  try {
                    let yes = confirm(
