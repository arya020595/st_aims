...
const QUERIES = gql`
  query listQueries($monthYear: String!, $filters: String) {
    countVegetableProduction(monthYear: $monthYear, filters: $filters)
  }
`;
...
const SEARCH_VEGETABLE_QUERY = gql`
...
const CREATE_VEGETABLE_PRODUCTION = gql`
  mutation tokenizedCreateManyVegetableProduction($tokenized: String!) {
    tokenizedCreateManyVegetableProduction(tokenized: $tokenized)
  }
`;

const UPDATE_VEGETABLE_PRODUCTION = gql`
  mutation tokenizedUpdateVegetableProduction($tokenized: String!) {
    tokenizedUpdateVegetableProduction(tokenized: $tokenized)
  }
`;

const DELETE_VEGETABLE_PRODUCTION = gql`
  mutation tokenizedDeleteVegetableProduction($tokenized: String!) {
    tokenizedDeleteVegetableProduction(tokenized: $tokenized)
  }
`;

const EXPORT_PRODUCTION = gql`
  mutation exportVegetableProduction(
    $monthYear: String!
    $farmerUUID: String
    $farmAreaId: String
...
const Production = () => {
  ...
  let [countVegetableProduction, setCountVegetableProduction] = useState(0);
  ...
  let pageCount = useMemo(() => {
    if (!countVegetableProduction) return 1;
    return Math.ceil(countVegetableProduction / pageSize);
  }, [countVegetableProduction, pageSize]);
  ...
  if (formData.farmerUUID) {
    ...
    const payload = { farmerUUID };
    tokenizedParams = jwt.sign(payload, TOKENIZE);
  } else if (exportFormData.farmerUUID) {
    farmerUUID = exportFormData.farmerUUID;
    const payload = { farmerUUID };
    tokenizedParams = jwt.sign(payload, TOKENIZE);
  }
  ...
  const [createVegetableProduction] = useMutation(CREATE_VEGETABLE_PRODUCTION);
  const [updateVegetableProduction] = useMutation(UPDATE_VEGETABLE_PRODUCTION);
  const [deleteVegetableProduction] = useMutation(DELETE_VEGETABLE_PRODUCTION);
  const [exportVegetableProduction] = useMutation(EXPORT_PRODUCTION);

  const [allFarmerProfilesByCompanyRegNo, setAllFarmerProfilesByCompanyRegNo] =
    useState([]);
  const [allVegetableProductions, setAllVegetableProductions] = useState([]);
  ...
  useEffect(async () => {
    ...
    if (!loading && !error) {
      ...
      // const encryptedVegetableProductions =
      //   data?.tokenizedAllVegetableProductions || "";
      // let allVegetableProductions = [];
      // if (encryptedVegetableProductions) {
      //   const decrypted = jwt.verify(encryptedVegetableProductions, TOKENIZE);
      //   allVegetableProductions = decrypted.queryResult;
      //   setAllVegetableProductions(allVegetableProductions);
      // }
      ...
      const countData = data?.countVegetableProduction || 0;
      setCountVegetableProduction(countData);
    }
    hideLoadingSpinner();
  ...
  useEffect(async () => {
    ...
    const countVegetableProduction = result.data?.countVegetableProduction || 0;
    setCountVegetableProduction(countVegetableProduction);

    const encryptedVegetableProductions =
      result.data?.tokenizedAllVegetableProductions || "";
    let allVegetableProductions = [];
    if (encryptedVegetableProductions) {
      const decrypted = jwt.verify(encryptedVegetableProductions, TOKENIZE);
      allVegetableProductions = decrypted.queryResult;
  ...
  const fethcingVegetable = async (input, callback) => {
    const result = await client.query({
      query: SEARCH_VEGETABLE_QUERY,
      variables: {
        name: input,
      },
      fetchPolicy: "no-cache",
    ...
    if (encryptedVegetable) {
      ...
      allVegetable = decrypted.queryResult;
      setAllCropVegetables(allVegetable);
    }
    callback(allVegetable);
  };
  ...
  const getVegetable = (input, callback) => {
    if (!input) {
      ...
      fethcingVegetable(input, callback);
    }
  };
  ...
  //   showLoadingSpinner();
  //   const result = await client.query({
  //     query: VEGETABLE_PRODUCTION_QUERY,
  //     variables: {
  //       monthYear: yearMonth,
  //     },
  //     fetchPolicy: "no-cache",
            ...
            <button
              onClick={async (e) => {
                ...
                setUpdateFormData({
                  ...
                  englishName: props.row.original?.Vegetable?.englishName || "",
                  localName: props.row.original?.Vegetable?.localName || "",
    ...
    <AdminArea header={{ title: "Production" }} urlQuery={router.query}>
      <FormModal
        ...
        onSubmit={async (e) => {
          ...
          try {
            ...
            const base64Response = response.data.exportVegetableProduction;
            ...
            link.href = url;
            link.download = "vegetable_production.xlsx";
            link.click();
            window.URL.revokeObjectURL(url);

            // window.open(response.data.exportVegetableProduction, "__blank");
          } catch (err) {
      ...
      >
        <div>
          ...
          <div className="form-group">
            <label>Vegetable Local Name</label>
            {/* <Select
              isClearable={true}
              className="form-control"
              ...
              classNamePrefix="select"
              onChange={(selectedValues) => {
                const found = allCropVegetables.find((p) =>
            ...
            /> */}
            <AsyncSelect
              value={exportFormData.Vegetable}
      ...
      <FormModal
        ...
        onSubmit={async (e) => {
          ...
          try {
            ...
            await updateVegetableProduction({
              variables: {
                tokenized,
                // ...updateFormData,
      ...
      >
          ...
          onClick={async (e) => {
            ...
            if (farmerCheck) {
              ...
              if (encryptedFarmProfile) {
                const decrypted = jwt.verify(encryptedFarmProfile, TOKENIZE);
                allFarmProfile = decrypted.queryResult;
                console.log("this all farm profile", allFarmProfile);
                setAllFarmProfile(allFarmProfile);
              }
        ...
        <div className="form-group">
          ...
          <input
            ...
            value={updateFormData.englishName || ""}
          />
        </div>
        ...
        <div className="form-group">
          ...
          <NumberFormat
            ...
            onValueChange={(e) => {
              // if (e) e.preventDefault();
              const val = parseFloat(e.formattedValue || 0);
      ...
      <FormModal
        ...
        onSubmit={async (e) => {
          ...
          try {
            ...
            const vegetableUUIDNotExist = records.some(
              (rec) => !rec.hasOwnProperty("vegetableUUID")
            );

            if (vegetableUUIDNotExist) {
            ...
            const tokenizedPayload = records.map((rec) => {
              ...
              return {
                ...
                vegetableUUID: rec.vegetableUUID,
                production: parseFloat(production || 0),
                cultivatedArea: parseFloat(cultivatedArea || 0),
                farmPrice: parseFloat(farmPrice || 0),
            ...
            await createVegetableProduction({
              variables: {
                tokenized,
              },
            });
      ...
      >
        ...
        {formData.farmerUUID && (
          ...
          >
            ...
            {formData.addresses.map((addr) => (
              <option value={addr}>{addr.toUpperCase()}</option>
            ))}
          </select> */}
            </div>
        ...
        {formData.records.map((rec) => (
          <div className="grid grid-cols-8 my-2 uppercase">
            <div className="pr-2">
              <AsyncSelect
                value={rec.Vegetable}
                loadOptions={getVegetable}
            ...
            <div className="pr-2">
              <NumberFormat
                ...
                onValueChange={(e) => {
                  const val = parseFloat(e.value || 0);
                  const totalFarmValue = val * rec.production;
      ...
      <div className="mt-26 pr-0 md:pr-10 py-4 h-full">
        <TableAsync
                ...
                <button
                  className={`bg-mantis-500 px-4 py-2 text-white rounded-md shadow-md ${
                    !currentUserDontHavePrivilege([
                      "Production Vegetable Export Excel:Read",
                    ])
                      ? "block"
                      : "hidden"
                  }`}
          ...
          onAdd={
            !currentUserDontHavePrivilege(["Production Vegetable:Create"])
              ? () => {
                  setFormData({
                    records: [],
                    addresses: [],
          ...
          onRemove={
            !currentUserDontHavePrivilege(["Production Vegetable:Delete"])
              ? async ({ rows }) => {
                  showLoadingSpinner();
                  try {
                    let yes = confirm(
                    ...
                    if (yes) {
                      for (const row of rows) {
                        ...
                        await deleteVegetableProduction({
                          variables: {
                            // uuid: row.uuid,
                            tokenized,
                          },
          ...
          customUtilities={
            !currentUserDontHavePrivilege(["Production Vegetable:Update"])
              ? customUtilities
              : null
          }
