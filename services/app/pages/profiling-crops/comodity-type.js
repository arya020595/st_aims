...
const ComodityType = () => {
  ...
  return (
    <AdminArea urlQuery={router.query}>
      <Head>
        <title>Master Data | Comodity Type</title>
      </Head>

      <FormModal
        title={`${!formData.uuid ? "New" : "Edit"} Comodity Type`}
      ...
      >
        <div className="form-group">
          <label>Comodity Type</label>
          <input
            className="form-control"
            value={formData.comodityType || ""}
            onChange={(e) => {
              if (e) e.preventDefault();
              setFormData({
                ...formData,
        ...
        <Table
          ...
          onAdd={
            !currentUserDontHavePrivilege([
              "Commodity Type Profiling Crops:Create",
            ])
              ? (e) => {
                if (e) e.preventDefault();
                setModalVisible(true);
          ...
          onRemove={
            !currentUserDontHavePrivilege([
              "Commodity Type Profiling Crops:Delete",
            ])
              ? async ({ rows }) => {
                showLoadingSpinner();
                try {
          ...
          customUtilities={
            !currentUserDontHavePrivilege([
              "Commodity Type Profiling Crops:Update",
            ])
              ? customUtilities
              : null
          }
