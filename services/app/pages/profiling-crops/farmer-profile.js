...
const QUERY = gql`
  query allFarmerProfilesByCompanyRegNo($filters: String) {
    ...
    countAllFarmerProfileByCompanyRegNo(filters: $filters)
  }
`;

const FARMER_PROFILE_QUERY = gql`
  query tokenizedAllFarmerProfilesByCompanyRegNo(
    $pageIndex: Int
    $pageSize: Int
    $filters: String
  ) {
...
const FarmerProfile = () => {
  ...
  // const allFarmerProfilesByCompanyRegNo =
  //   data?.allFarmerProfilesByCompanyRegNo || [];
  ...
  let [countFarmerProfile, setCountFarmerProfile] = useState(0);
  ...
  let pageCount = useMemo(() => {
    if (!countFarmerProfile) return 1;
    return Math.ceil(countFarmerProfile / pageSize);
  }, [countFarmerProfile, pageSize]);

  useEffect(() => {
    if (!error && !loading) {
      // let allFarmerProfilesByCompanyRegNo = [];
      // const encrypted = data?.tokenizedAllFarmerProfilesByCompanyRegNo || "";
      // if (encrypted) {
      ...
      const countData = data?.countAllFarmerProfileByCompanyRegNo || 0;
      setCountFarmerProfile(countData);
    }
  ...
  useEffect(async () => {
    ...
    const encryptedFarmerProfilesByCompanyRegNo =
      result.data?.tokenizedAllFarmerProfilesByCompanyRegNo || "";
    const countData = result.data?.countAllFarmerProfileByCompanyRegNo || 0;
    setCountFarmerProfile(countData);
    let allFarmerProfilesByCompanyRegNo = [];
    ...
    <AdminArea urlQuery={router.query}>
      ...
      >
        ...
        <div className="my-2">
          ...
          <div className="grid grid-cols-3 gap-3">
            {allSupportTypes.map((suppSystem) => {
              let isChecked = false;

              const foundIndex = formData.supportSystem.findIndex(
                (idx) => idx === suppSystem.uuid
              );
              ...
              return (
                <div>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (foundIndex === -1) {
                        ...
                        setFormData({
                          ...formData,
                          supportSystem: formData.supportSystem.filter(
                            (p) => p !== suppSystem.uuid
                          ),
                        });
                      }
                  ...
                  />
                  <label>{suppSystem.supports}</label>
                </div>
              );
            })}
          </div>
      ...
      <div className="mt-26 pr-0 md:pr-10 py-4 h-full">
        <Table
          ...
          columns={columns}
          data={
            !currentUserDontHavePrivilege(["Farmer Profiling Crops:Read"])
              ? allFarmerProfilesByCompanyRegNo
              : []
          }
          ...
          customHeaderUtilities={
            !currentUserDontHavePrivilege([
              "Farmer Profiling Crops Export Excel:Create",
            ]) ? (
          ...
          withoutHeader={true}
          onAdd={
            !currentUserDontHavePrivilege(["Farmer Profiling Crops:Create"])
              ? async (e) => {
                  if (e) e.preventDefault();
          ...
          onRemove={
            !currentUserDontHavePrivilege(["Farmer Profiling Crops:Delete"])
              ? async ({ rows }) => {
                  showLoadingSpinner();
                  try {
                    let yes = confirm(
          ...
          customUtilities={
            !currentUserDontHavePrivilege(["Farmer Profiling Crops:Update"])
              ? customUtilities
              : null
          }
