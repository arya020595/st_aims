...
const CREATE_CONTRACT_STATUS = gql`
  mutation tokenizedCreateContractStatus($tokenized: String!) {
    tokenizedCreateContractStatus(tokenized: $tokenized)
  }
`;

const UPDATE_CONTRACT_STATUS = gql`
  mutation tokenizedUpdateContractStatus($tokenized: String!) {
    tokenizedUpdateContractStatus(tokenized: $tokenized)
  }
`;

const DELETE_CONTRACT_STATUS = gql`
  mutation tokenizedDeleteContractStatus($tokenized: String!) {
    tokenizedDeleteContractStatus(tokenized: $tokenized)
  }
`;
const ContractStatus = () => {
  ...
  const [createContractStatus] = useMutation(CREATE_CONTRACT_STATUS);
  const [updateContractStatus] = useMutation(UPDATE_CONTRACT_STATUS);
  const [deleteContractStatus] = useMutation(DELETE_CONTRACT_STATUS);
    ...
    <AdminArea urlQuery={router.query}>
      ...
      <div className="mt-26 pr-0 md:pr-10 py-4 h-full">
        <Table
          loading={loading}
          columns={columns}
          data={allContractStatuses}
          withoutHeader={true}
          onAdd={
            !currentUserDontHavePrivilege([
              "Contract Status Profiling Livestock:Create",
            ])
              ? (e) => {
                  if (e) e.preventDefault();
                  setModalVisible(true);
          ...
          onRemove={
            !currentUserDontHavePrivilege([
              "Contract Status Profiling Livestock:Delete",
            ])
              ? async ({ rows }) => {
                  showLoadingSpinner();
                  try {
          ...
          customUtilities={
            !currentUserDontHavePrivilege([
              "Contract Status Profiling Livestock:Update",
            ])
              ? customUtilities
              : null
          }
...
export default withApollo({ ssr: true })(ContractStatus);
