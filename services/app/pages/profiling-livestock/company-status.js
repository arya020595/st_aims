...
const QUERY = gql`
  query tokenizedAllCompanyStatus {
...
const CompanyStatus = () => {
  ...
  const [createCompanyStatusTokenized] = useMutation(CREATE_COMPANY_STATUS);
  const [updateCompanyStatusTokenized] = useMutation(UPDATE_COMPANY_STATUS);
  const [deleteCompanyStatusTokenized] = useMutation(DELETE_COMPANY_STATUS);
  ...
  // const encrypted = data?.tokenizedAllCompanyStatus || "";
  // let allCompanyStatuses = [];
  // if (encrypted) {
  //   const decyrpted = jwt.verify(encrypted, TOKENIZE);
  //   allCompanyStatuses = decyrpted.queryResult;
  // }
    ...
    <AdminArea urlQuery={router.query}>
        ...
        onSubmit={async (e) => {
          ...
          try {
            ...
            if (!uuid) {
              ...
              await createCompanyStatusTokenized({
                variables: {
                  tokenized,
                },
              });
              ...
              const tokenized = jwt.sign(formData, TOKENIZE);
              await updateCompanyStatusTokenized({
                variables: {
                  tokenized,
                },
              });
      ...
      >
        ...
        <div className="form-group">
          ...
          <textarea
            className="form-control"
            value={(formData.descriptionMalay || "").toUpperCase()}
            onChange={(e) => {
              if (e) e.preventDefault();
              setFormData({
                ...formData,
      ...
      <div className="mt-26 pr-0 md:pr-10 py-4 h-full">
        <Table
          ...
          withoutHeader={true}
          onAdd={
            !currentUserDontHavePrivilege(["Company Status Livestock:Create"])
              ? (e) => {
                  if (e) e.preventDefault();
                  setModalVisible(true);
                  setFormData({});
          ...
          onRemove={
            !currentUserDontHavePrivilege(["Company Status Livestock:Delete"])
              ? async ({ rows }) => {
                  showLoadingSpinner();
                  try {
                    let yes = confirm(
                    ...
                    if (yes) {
                      for (const row of rows) {
                        ...
                        await deleteCompanyStatusTokenized({
                          variables: {
                            tokenized,
                          },
                        });
          ...
          customUtilities={
            !currentUserDontHavePrivilege(["Company Status Livestock:Update"])
              ? customUtilities
              : null
          }
