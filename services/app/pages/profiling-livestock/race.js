...
const Race = () => {
  ...
  // const encryptedRace = data?.tokenizedAllRace || "";
  // let allRaces = [];
  // if (encryptedRace) {
  //   const decrypted = jwt.verify(encryptedRace, TOKENIZE);
  //   allRaces = decrypted.queryResult;
  // }
    ...
    <AdminArea urlQuery={router.query}>
      ...
      >
        <div className="form-group">
          ...
          <input
            ...
            value={(formData.race || "").toUpperCase()}
            onChange={(e) => {
              if (e) e.preventDefault();
              setFormData({
                ...formData,
      ...
      <div className="mt-26 pr-0 md:pr-10 py-4 h-full">
        <Table
          ...
          withoutHeader={true}
          onAdd={
            !currentUserDontHavePrivilege(["Race Profiling Livestock:Create"])
              ? (e) => {
                  if (e) e.preventDefault();
                  setModalVisible(true);
                  setFormData({});
          ...
          onRemove={
            !currentUserDontHavePrivilege(["Race Profiling Livestock:Delete"])
              ? async ({ rows }) => {
                  showLoadingSpinner();
                  try {
                    let yes = confirm(
          ...
          customUtilities={
            !currentUserDontHavePrivilege(["Race Profiling Livestock:Update"])
              ? customUtilities
              : null
          }
