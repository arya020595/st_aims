...
const IMPORT_LIVESTOCK_RETAIL_PRICE = gql`
  mutation importRetailPriceLivestock(
    $excelBase64: String!
    $year: Int!
    $month: Int!
    $fileName: String!
...
const GENERATE_TEMPLATE = gql`
  mutation generateRetailPriceLivestock {
    generateRetailPriceLivestock
  }
`;
...
const LivestockRetailImport = ({}) => {
  const notification = useNotification();
  const [importRetailPriceLivestock] = useMutation(
    IMPORT_LIVESTOCK_RETAIL_PRICE
  );
  const [generateRetailPriceLivestock] = useMutation(GENERATE_TEMPLATE);
  ...
  const handleDrop = (acceptedFiles) => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        await importRetailPriceLivestock({
          variables: {
        ...
        <div className=" flex justify-center my-10">
          <button
            ...
            onClick={async () => {
              showLoadingSpinner();
              try {
                const result = await generateRetailPriceLivestock({
                  variables: {},
                });
                // Convert base64 to blob
                const base64Response = result.data.generateRetailPriceLivestock;
                ...
                // window.open(
                //   result.data.generateRetailPriceLivestock,
                //   "__blank"
                // );
              } catch (err) {
                notification.handleError(err);
...
export default withApollo({ ssr: true })(LivestockRetailImport);
