...
const IMPORT_FRUIT_RETAIL_PRICE = gql`
  mutation importFruitRetailPrice(
    $excelBase64: String!
    $year: Int!
    $month: Int!
    $fileName: String!
...
const GENERATE_FRUIT_RETAIL_TEMPLATE = gql`
  mutation generateFruitRetailPriceTemplate {
    generateFruitRetailPriceTemplate
  }
`;
const MONTHSLIST = [
...
const VegetableExcelImport = ({}) => {
  ...
  const [importFruitRetailPrice] = useMutation(IMPORT_FRUIT_RETAIL_PRICE);
  const [generateFruitRetailPriceTemplate] = useMutation(
    GENERATE_FRUIT_RETAIL_TEMPLATE
  );
        ...
        <div className=" flex justify-center my-10">
          <button
            ...
            onClick={async () => {
              showLoadingSpinner();
              try {
                const result = await generateFruitRetailPriceTemplate({
                  variables: {},
                });

                // Convert base64 to blob
                const base64Response = result.data.generateFruitRetailPriceTemplate;
                ...
                link.href = url;
                link.download = 'fruit_retail_price_template.xlsx';
                link.click();
                window.URL.revokeObjectURL(url);

                
                // window.open(
                //   result.data.generateFruitRetailPriceTemplate,
                //   "__blank"
                // );
              } catch (err) {
                notification.handleError(err);
