...
const IMPORT_MISCELLANEOUS_RETAIL_PRICE = gql`
  mutation importMiscellaneousRetailPrice(
    $excelBase64: String!
    $year: Int!
    $month: Int!
    $fileName: String!
...
const GENERATE_MISC_RETAIL_TEMPLATE = gql`
  mutation generateMiscellaneousCropsRetailPriceTemplate {
    generateMiscellaneousCropsRetailPriceTemplate
  }
`;

const VegetableExcelImport = ({}) => {
  const [importVegetableRetailPrice] = useMutation(
    IMPORT_MISCELLANEOUS_RETAIL_PRICE
  );
  const [generateMiscellaneousCropsRetailPriceTemplate] = useMutation(
    GENERATE_MISC_RETAIL_TEMPLATE
  );
        ...
        <div className=" flex justify-center my-10">
          <button
            ...
            onClick={async () => {
              ...
              try {
                const result =
                  await generateMiscellaneousCropsRetailPriceTemplate({
                    variables: {},
                  });

                // Convert base64 to blob
                const base64Response =
                  result.data.generateMiscellaneousCropsRetailPriceTemplate;
